...
parser.add_argument('-W', '--websites',
                    action='store_true',
                    help='Input is a list of websites')
...
if args.websites and args.portdata:
    ...
    try:
        pd.DataFrame(externals, columns=['Website', 'IP']).set_index('Website').to_csv('./Output/Externals.csv',
                                                                                       header=True, sep=',')
    except Exception as e:
        print('\nError exporting a list of websites that were hosted on external IP\'s:', str(e))
    else:
...
siteciphersSSLv2, siteciphersSSLv3, siteciphersTLS10, siteciphersTLS11, siteciphersTLS12, wcsites, packages, exceptions, vulnerable = CipherScan.cscan(
    sslfiltered, vulnerable, siteDict)

securesites = ExportXML.tree(packages, siteDict, siteciphersSSLv2, siteciphersSSLv3, siteciphersTLS10, siteciphersTLS11,
                             siteciphersTLS12, exceptions)
